#!/bin/ksh
################################################################################
# File   : fixCorruptUser.sh 
# Author : Ronald Shiou
# Date   : 12/05/2014
#
# Updated: 12/05/2014 - new script
#          
#
# Usage  : fixCorruptUser.sh -d <ORACLE_SID> -s <SCHEMA_NAME> [-m DISTRIB_LIST] 
#
# Descrip: call sql script as the SCHEMA_NAME user to fix corrupt users
################################################################################
...
if [ ! "$SCHEMA_NAME" ]
then
	echo "(`hostname`): CRITICAL: <schema_name> did not past into running `basename $0`" | mail -s "(`hostname`): `basename $0`: FAILURE on $ORACLE_SID" $TO_EMAIL
	exit 1
fi
...
PASSWD_FILE=$BASE_DIR/admin/.${SCHEMA_NAME}_${ORACLE_SID}
...
SQL_SCRIPT="$BASE_DIR/sql/fix_corrupt_user.sql"
