#!/bin/ksh
################################################################################
# File   : report_acfs_space.sh
# Author : Ronald Shiou 
# Date   : 06/30/2016
#
#
# Usage  : report_acfs_space.sh 
#
 # Descrip: 
################################################################################
...
create_logfile
echo "REDO: /u01/app/oracle/oradata/datastore"
echo "DATA: /u02/app/oracle/oradata/datastore"
echo ""
...
#spool ${LOGFILE}
#select name, round(total_mb/4/1024,2) as total_gb, round(usable_file_mb/1024,2) as usable_file_gb, round((usable_file_mb/(total_mb/4))*100,2) as FREE_PCT  from v\$asm_diskgroup where name <> 'REDO' order by 1;
...
#if [ $SQL_RC -eq 0 ]
#then
   cat $LOGFILE | mail -s "Report: ACFS Space Usage X5: ${HOST}" $TO_EMAIL
#else
