#!/bin/ksh
################################################################################
# File   : expdp_pdb.oci.sh
# Author : Ronald Shiou
# Date   : 01/18/2022
#
# Usage  : expdp_pdb.oci.sh -d <ORACLE_SID> -c <CONNECT AS USER> -m [EMAIL] -s <SCHEMA TO EXPORT>
#
# Note:	 This script is modified to be compatible with OCI PDBs
...
#  ***    2) make sure database directory exists (OCI_DUMP_DIR) 
#         3) schedule cron job like this for exmaple:
#             00 23 * * * /usr/local/bin/oracle/scripts/expdp_pdb.oci.sh -d EAVE215D17 -c c##nfidba -m nfii-dba-admin@nfiindustries.com -s wmlm215eave,msf215eave 
#        
################################################################################
...
NUM_ORA_ERRS=$(expr $(grep "ORA-" $LOGFILE | grep -v "39173"|  wc -l) )
## new error in 19c
...
FAILURE_MESSAGE="expdp_pdb.oci.sh -d <ORACLE_SID> -c <CONNECT AS USER TO EXPORT> -d <DB> -m [EMAIL] -s <SCHEMA TO EXPORT>"
...
select DIRECTORY_PATH from dba_directories where DIRECTORY_NAME = 'OCI_DUMP_DIR' ;
!`
check_err $?
...
if [[ $SCHEMANAME == "FULL" ]]
then
${ORACLE_HOME}/bin/expdp ${USERNAME}/${PASSWD}@${ORACLE_SID} DUMPFILE=$DUMP_FILE_U DIRECTORY=OCI_DUMP_DIR FULL=y PARALLEL=4 LOGFILE=$EXP_LOG_FILE
else
${ORACLE_HOME}/bin/expdp ${USERNAME}/${PASSWD}@${ORACLE_SID} DUMPFILE=$DUMP_FILE_U DIRECTORY=OCI_DUMP_DIR SCHEMAS=$USERINPUT_SCHEMANAME PARALLEL=4 LOGFILE=$EXP_LOG_FILE 
fi
